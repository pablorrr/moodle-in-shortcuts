...
echo html_writer::nonempty_tag('h4', 'Detective case #1: Find out what role is assigned to Robert Johnson.'); 
...
echo html_writer::nonempty_tag('h4', 'Detective case #2: Find all users assigned to the coursecreator role.'); 

$getrole = $DB->get_record_select('role', 'shortname = ?', array('coursecreator'));

echo 'The id for the coursecreator role is ';
echo $getrole->id; 
echo '<br>';

$getmembers = $DB->get_records_select('role_assignments', 'roleid = ?', array($getrole->id));

echo 'get_records_select function returns: ';
print_r($getmembers); 
echo '<br>'; 

foreach($getmembers as $showmembers) {
	
	$getuserinfo = $DB->get_record_select('user', 'id = ?', array($showmembers->userid)); 
	
	echo 'Member name: ';
	echo $getuserinfo->firstname;
	echo ' ';
	echo $getuserinfo->lastname; 
	echo '<br>'; 
